      ...
      <div className="w-full max-w-sm bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div className="px-6 py-6 flex items-center gap-4 border-b border-gray-100">
          ...
          <div>
            ...
            <span className={`inline-block text-sm px-3 py-0.5 rounded-full font-semibold mt-1 ${c.status === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>{c.status}</span>
          </div>
        </div>
        ...
        <div className="px-6 py-5 grid grid-cols-2 gap-3">
          ...
          <div className={`${c.totalDue > 0 ? "bg-red-50 border-red-100" : "bg-gray-50 border-gray-100"} border rounded-xl p-4 col-span-2`}>
            <div className={`text-xl font-bold ${c.totalDue > 0 ? "text-red-600" : "text-gray-400"}`}>{c.totalDue > 0 ? fmt(c.totalDue) : "No due ✅"}</div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-7">
        <div className="max-w-6xl mx-auto flex items-center gap-4">
          ...
          <div className="ml-auto bg-green-100 text-green-700 text-xl font-bold px-5 py-2 rounded-2xl">{data.length} total</div>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {rows.length === 0 ? (
                  ...
                  <tr key={c.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-5 py-4">{c.totalDue > 0 ? <span className="bg-red-100 text-red-600 text-base font-semibold px-3 py-1 rounded-lg">{fmt(c.totalDue)}</span> : <span className="text-gray-300">—</span>}</td>
            ...
            {rows.length === 0 ? (
              ...
              <div key={c.id} className="p-5">
                <div className="flex items-start justify-between gap-2 mb-4">
                  ...
                  <span className={`text-sm font-semibold px-2.5 py-1 rounded-lg capitalize ${c.status === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>{c.status}</span>
                </div>
                <div className="grid grid-cols-3 gap-2 mb-4">
                  ...
                  <div className={`${c.totalDue > 0 ? "bg-red-50" : "bg-gray-50"} rounded-xl p-3 text-center`}><div className={`text-sm font-bold ${c.totalDue > 0 ? "text-red-600" : "text-gray-400"}`}>{c.totalDue > 0 ? fmt(c.totalDue) : "—"}</div><div className="text-xs text-gray-400 mt-0.5">Due</div></div>
                </div>
